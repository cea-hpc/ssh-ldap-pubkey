...
class LdapSSH(object):
    ...
    def find_and_update_pubkey(self, login, password, ukey):
        """Find and update public keys of the user with ``login`` that have the same raw key as ``ukey``.
    ...
    def _filter(self, pubkeys, validity=None, expire=None):
        ...
        for key in pubkeys:
            ...
            if validity == 'invalid' and (expiration is None):
                ...
                filtered.append(key)
            elif validity == 'expire' and (expiration is not None and expiration >= today and day > expiration):
                filtered.append(key)
